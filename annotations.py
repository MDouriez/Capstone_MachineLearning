...
fileIn = 'Annotations Capstone.xlsx'
...
df_bed['Activity'] = df_bed['Activity'].replace(['C','B'],['Change','Sleep'])
...
start_date = '25/02/2016 00:01:00'
days=14
periods = days*24*60 # numer of periods: 2 weeks with a frequency of 1 min
rng = pd.date_range(start_date, periods=periods, freq='min')
...
for i in range(0,len(df_kitch.index)):
    ...
    df.loc[time_index,'Pres_kitch'] = 1


y = df['Activity']
presence = df[['Pres_bath', 'Pres_kitch', 'Pres_bed']]
presence.to_csv("presence.csv")
y.to_csv('y.csv')
